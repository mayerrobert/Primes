  ...
  (define-vop (data-vector-set-with-offset/simple-bit-vector)
    ...
    (:generator 24
      (sc-case value
        (immediate
          (unless +use-btrbts+
            ; variant 1: use and/or
            ; and qword-ptr[index >> 6] 1<<(index & 0x3f)
            ; or qword-ptr[index >> 6] ~(1<<(index & 0x3f))
            (aver (zerop offset))
          ...
          (inst mov result (tn-value value))) ; not sure if this is neccessary

          (when +use-btrbts+
            ; variant 2: use bts/btr
            (aver (zerop offset))
